		...
		__le32 disposition)
{
	...
	if (((access & FILE_READ_DATA_LE || access & FILE_GENERIC_READ_LE) &&
		...
		oflags |= O_RDONLY;
	else if (access & (FILE_WRITE_DATA_LE | FILE_DELETE_LE |
			FILE_WRITE_ATTRIBUTES_LE | FILE_GENERIC_WRITE_LE))
		oflags |= O_WRONLY;
	else
		oflags |= O_RDONLY;
			    ...
			    struct ksmbd_file *fp)
{
	...
	rc = ksmbd_vfs_setxattr(path->dentry,
				...
				0);
	if (rc)
		ksmbd_debug(SMB, "failed to store file attribute in EA\n");
			       ...
			       struct ksmbd_share_config *share)
{
	...
	if (file_info->Attributes) {
		...
		if (test_share_config_flag(share,
			...
			if (rc)
				ksmbd_debug(SMB,
					"failed to store file attribute in EA\n");
			rc = 0;
		}
	}
