...
void smb2_set_err_rsp(struct ksmbd_work *work)
{
	...
	if (err_rsp->hdr.Command == SMB2_CREATE_HE && err_rsp->hdr.Status == STATUS_STOPPED_ON_SYMLINK) {
		struct smb2_error_context_rsp *err_cxt_rsp =
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 4, 0)
	rc = ksmbd_vfs_kern_path_locked(work, name, flags,
					&parent_path, &path, 1);
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(6, 4, 0)
	rc = ksmbd_vfs_kern_path_locked(work, link_name, flags, &parent_path,
					&path, 0);
#else
